    ...
    for (const auto& day : m_days) {
        for (const auto& meal : day.meals()) {
            ...
            for (const auto& recipe_ingredient : recipe_ingredients) {
                unsigned amount = 0;
                std::pair<ingredient::Ingredient, std::optional<recipe::Quantity>> ing = std::make_pair(recipe_ingredient.first, std::nullopt);
                ...
                if (entry != mapper.end()) {
                    ...
                    std::pair<ingredient::Ingredient, std::vector<std::optional<recipe::Quantity>>> new_ing = std::make_pair(ing.first, vec);
